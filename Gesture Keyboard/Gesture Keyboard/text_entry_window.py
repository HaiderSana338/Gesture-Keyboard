...
class Application(tk.Frame):
    ...
    def mouse_left_button_double_click(self, event):
        characters = self.label_word_candidates[0].cget(
            "text")  # get the existing characters in the first word candidate label
        ...
        if len(result) > 0:
            for i in range(len(result)):
                if i < len(self.label_word_candidates):
                    ...
                    break
                    # example commands from list
                    example_commands = ['copy', 'redo', 'undo', 'save']
                    if result[0][1] in example_commands:
                        tmsg.showinfo("Message Box", result[0][1])
                    else:
                        tmsg.showinfo("Message Box", "Not a command")

        else:
            key = self.keyboard.get_key_pressed()
            if key == 'Space':  # Create space bw letter or words
                self.text.insert('end', " ")
            else:
                key = self.keyboard.get_key_pressed()
                if key == 'Clear':  # Create space bw letter or words
                    ...
                    if length > 0:
                        ...
                        self.label_word_candidates[0].config(text=characters)  # only one key was pressed
